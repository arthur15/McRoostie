...
{
    public partial class VentasTostadas : Form
    {
        List<OrderDetails> orderComplete = new List<OrderDetails>();

        public VentasTostadas()
        {
            InitializeComponent();
        }
        ...
        private void AddProducts()
        {
            try
            {
                ...
                decimal QtyTostadaCa, QtyTostadaPo;

                QtyTostadaCa = Convert.ToDecimal(Convert.ToDecimal(lblTostadaCa.Text == "" ? "0" : lblTostadaCa.Text));
                QtyTostadaPo = Convert.ToDecimal(Convert.ToDecimal(lblTostadaPo.Text == "" ? "0" : lblTostadaPo.Text));
                //QtyHamb = Convert.ToDecimal(Convert.ToDecimal(txbHamb.Text == "" ? "0" : txbHamb.Text));

                if (QtyTostadaCa > 0)
                    orderComplete.Add(FillOrder("TC-Tostada", QtyTostadaCa));
                if (QtyTostadaPo > 0)
                    orderComplete.Add(FillOrder("TP-Tostada", QtyTostadaPo));
                //if (QtyTorta > 0)
